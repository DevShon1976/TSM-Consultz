...
const GuidedMission = (function () {
  ...
  const PATIENT_DATA = {
    ...
    operations: {
      ...
      claim: null,
      objectives: {
        0: { node: 'operations', instruction: 'Review the OR utilization report. Identify underperforming blocks and recommend corrective action.', fields: [{ label: 'Total OR Blocks Scheduled', value: '48', editable: false }, { label: 'Blocks Utilized', value: '31', editable: false }, { label: 'Utilization Rate', value: '', editable: true, correct: '64.6%', hint: '31 / 48 = ?' }, { label: 'Target Utilization', value: '80%', editable: false }, { label: 'Corrective Action', value: '', editable: true, correct: 'Release underutilized blocks 72hrs prior and open to add-on cases', hint: 'What is the standard action for blocks below 70% utilization?' }], action: { label: 'Submit Utilization Plan', validates: true }, success: 'OR utilization plan submitted. Underperforming blocks flagged for release. Add-on case protocol activated.', learn: 'OR utilization below 70% represents direct revenue loss. Standard practice is to release unused blocks 48-72 hours prior so other surgeons can schedule add-on cases. Every unused OR hour costs $1,500-$2,500 in lost contribution margin.' },
        1: { node: 'operations', instruction: 'A staffing variance report shows 3 departments over budget on overtime. Identify the highest risk department and recommend action.', fields: [{ label: 'ED Overtime Hours', value: '284 hrs — $18,460', editable: false }, { label: 'ICU Overtime Hours', value: '112 hrs — $7,280', editable: false }, { label: 'Surgical Services OT', value: '341 hrs — $22,165', editable: false }, { label: 'Highest Risk Dept', value: '', editable: true, correct: 'Surgical Services — $22,165 OT, highest variance', hint: 'Which department has the highest overtime dollar variance?' }, { label: 'Root Cause Action', value: '', editable: true, correct: 'Audit scheduling pattern — check for shift gaps and PRN pool underutilization', hint: 'What is the first step in resolving overtime variance?' }], action: { label: 'Submit Variance Response', validates: true }, success: 'Overtime variance escalated. Surgical Services scheduling audit initiated. PRN pool review queued.', learn: 'Overtime variance in surgical services is often caused by case overruns and inadequate PRN coverage. A scheduling pattern audit identifies whether the issue is structural (chronic understaffing) or operational (poor block management).' }
      }
    },
    staffing: {
      patient: null,
      claim: null,
      objectives: {
        0: { node: 'staffing', instruction: 'Review the credentialing queue. Three providers are past their reappointment deadline. Identify the compliance risk.', fields: [{ label: 'Provider 1', value: 'Dr. A. Patel — Expired 30 days ago', editable: false }, { label: 'Provider 2', value: 'Dr. R. Kim — Expired 14 days ago', editable: false }, { label: 'Provider 3', value: 'Dr. L. Torres — Expires in 3 days', editable: false }, { label: 'Compliance Risk', value: '', editable: true, correct: 'Billing for services by expired providers is a CMS compliance violation — suspend billing immediately', hint: 'What is the CMS rule for billing with lapsed provider credentials?' }, { label: 'Urgent Action', value: '', editable: true, correct: 'Suspend clinical privileges and halt billing for Patel and Kim pending reappointment', hint: 'What action is required for providers 30+ days past reappointment?' }], action: { label: 'Submit Credentialing Alert', validates: true }, success: 'Credentialing alert submitted. Patel and Kim flagged for privilege suspension. Torres fast-tracked for 3-day renewal.', learn: 'Billing Medicare/Medicaid for services rendered by a provider with lapsed credentials is a false claims violation. Suspend billing immediately and complete expedited reappointment. Never let credentials lapse — build a 60-day advance alert into your credentialing system.' },
        1: { node: 'staffing', instruction: 'A 90-day turnover report shows RN turnover at 28%. Calculate the financial impact and identify a retention action.', fields: [{ label: 'RN Turnover Rate', value: '28%', editable: false }, { label: 'Total RN FTEs', value: '142', editable: false }, { label: 'Avg Replacement Cost', value: '$52,000 per RN', editable: false }, { label: 'Annual Turnover Cost', value: '', editable: true, correct: '$2,065,600 (142 x 28% x $52,000)', hint: 'Calculate total annual RN replacement cost' }, { label: 'Top Retention Action', value: '', editable: true, correct: 'Implement stay interviews and flexible scheduling for highest flight-risk units', hint: 'What is the highest-ROI retention intervention for clinical staff?' }], action: { label: 'Submit Retention Plan', validates: true }, success: 'RN retention plan submitted. Stay interview program initiated. Scheduling flexibility review queued for top 3 flight-risk units.', learn: 'At 28% turnover, a 142-RN team costs over $2M annually in replacement. Stay interviews — asking staff why they stay and what would make them leave — have a higher ROI than exit interviews because they are preventive. Pair with scheduling flexibility for the highest-impact units.' }
      }
    },
