...
echo "========================================"
echo "TSM CLOSED LOOP EXECUTION SYSTEM v1"
echo "========================================"

FILE="html/core/tsm-bnca-closed-loop.js"

mkdir -p html/core

echo "[1] Writing Closed Loop BNCA Engine..."
...
echo "[2] Wiring into portal..."

PORTAL="html/construction-suite/construction-executive-portal.html"

if [ -f "$PORTAL" ]; then
  grep -q "tsm-bnca-closed-loop.js" "$PORTAL" || cat <<'HTML' >> "$PORTAL"

<!-- TSM CLOSED LOOP EXECUTION SYSTEM -->
<script defer src="/core/tsm-bnca-closed-loop.js"></script>
...
git add -A
git commit -m "BNCA Closed Loop Execution System v1" || echo "No changes"
...
echo "========================================"
echo "✔ CLOSED LOOP SYSTEM INSTALLED"
