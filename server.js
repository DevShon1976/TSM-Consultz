...
const COLLECTIVE_VERTICALS = ['healthcare', 'finops', 'bpo', 'legal', 'real-estate', 'insurance', 'construction', 'o2c', 'crm', 'cpq', 'mdm', 'integration', 'governance', 'digital-twin'];
...
app.get('/api/mdm/analysis/:domain', (req, res) => {
  ...
  if (!records) return res.status(404).json({ ok: false, error: "Unknown domain. Valid domains: customer, vendor, gl" });
  res.json({
    ok: true,
    domain,
